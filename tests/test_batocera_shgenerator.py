...
"""
Tests for PortMaster/batocera/shGenerator.py
...
class TestBatoceraShGenerator(unittest.TestCase):
    """Test Batocera shGenerator module structure"""

    def setUp(self):
        """Set up test file path"""
        self.sh_generator_file = Path(__file__).parent.parent / 'PortMaster' / 'batocera' / 'shGenerator.py'
    ...
    def test_has_gethotkeyscontext_method(self):
        """Test file contains getHotkeysContext method"""
        content = self.sh_generator_file.read_text()
        self.assertIn('def getHotkeysContext(', content)
